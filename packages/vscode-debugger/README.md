...
- **Events** — every event each actor receives. Press **⌘F / Ctrl+F** (or the search icon in the panel title) for a find widget that filters the log by event type, actor name, or event payload, with match-case / whole-word / regex toggles and **↑ / ↓** to step through matches. Space-separated terms are AND-ed and a `-` prefix excludes (e.g. `fetch -TICK`). Click an event to inspect it and pause the log on that row — the Instances tree reveals the actor it hit, and the Context / Event trees follow. With the log focused, **← / →** step to the previous / next event and **Esc** returns to live. Also **Step Back / Forward**, **Back to Live**, and **Clear**. Stepping freezes the Instances + Context trees at that point.
