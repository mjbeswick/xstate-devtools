...
export function AuthForm() {
  ...
  return (
    <div style={{ border: '1px solid #eee', padding: 16, borderRadius: 8, maxWidth: 420 }}>
      <h3>
        Auth Machine — state: <code>{JSON.stringify(state.value)}</code>
      </h3>
      ...
      {(state.matches('idle') || state.matches('failed')) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
          ...
          <div style={{ fontSize: 11, color: '#888' }}>
            Use password "wrong" to trigger the error state. MFA code: 123456
          </div>
        </div>
      )}
      ...
      {state.matches('authenticated') && (
        <div style={{ marginTop: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            <NavButton
              active={state.matches({ authenticated: { active: 'home' } })}
              onClick={() => send({ type: 'VIEW_HOME' })}
            >
              Home
            </NavButton>
            <NavButton
              active={state.matches({ authenticated: { active: 'profile' } })}
              onClick={() => send({ type: 'VIEW_PROFILE' })}
            >
              Profile
            </NavButton>
            <NavButton
              active={state.matches({ authenticated: { active: 'settings' } })}
          ...
          {state.matches({ authenticated: { active: 'settings' } }) && (
            <div style={{ border: '1px solid #ddd', borderRadius: 6, padding: 8, marginBottom: 8 }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                <NavButton
                  active={state.matches({ authenticated: { active: { settings: 'general' } } })}
                  onClick={() => send({ type: 'TAB_GENERAL' })}
                >
                  General
                </NavButton>
                <NavButton
                  active={state.matches({ authenticated: { active: { settings: 'security' } } })}
                  onClick={() => send({ type: 'TAB_SECURITY' })}
                >
                  Security
                </NavButton>
                <NavButton
                  active={state.matches({ authenticated: { active: { settings: 'billing' } } })}
                  onClick={() => send({ type: 'TAB_BILLING' })}
                >
                  Billing
                </NavButton>
              </div>
              ...
              {state.matches({ authenticated: { active: { settings: 'security' } } }) && (
                <div style={{ paddingLeft: 8, fontSize: 13 }}>
                  <div style={{ marginBottom: 4 }}>
                    ...
                    <strong>
                      {state.matches({
                        authenticated: { active: { settings: { security: 'twoFactor' } } },
                      })
                        ? 'Enabled'
                        : 'Disabled'}
                    </strong>
