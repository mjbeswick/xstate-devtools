...
export function ShoppingCart() {
  ...
  return (
    <div style={{ border: '1px solid #eee', padding: 16, borderRadius: 8 }}>
      <h3>
        Cart Machine — checkout: <code>{JSON.stringify(state.value.checkout)}</code>
      </h3>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        {ITEMS.map((item) => (
          <button
            key={item.id}
            onClick={() => send({ type: 'ADD_ITEM', item })}
            style={{ margin: '0 4px' }}
          >
            + {item.name}
          </button>
      ...
      {state.context.paymentMethod && (
        <p style={{ fontSize: 13, color: '#666' }}>
          Payment: <strong>{state.context.paymentMethod}</strong>
        </p>
      )}
      {state.context.promoCode && (
        <p style={{ fontSize: 13, color: '#666' }}>
          Promo: <strong>{state.context.promoCode}</strong>
        </p>
      )}
      ...
      {state.matches({ checkout: { details: 'reviewing' } }) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button onClick={() => send({ type: 'APPLY_PROMO', code: 'SAVE10' })}>
            Apply promo SAVE10
          </button>
        ...
        <div style={{ border: '1px solid #ddd', borderRadius: 6, padding: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            <button
              ...
              style={{
                background: state.matches({ checkout: { details: { choosingPayment: 'card' } } })
                  ? '#1890ff'
                  : '#f5f5f5',
                color: state.matches({ checkout: { details: { choosingPayment: 'card' } } })
                  ? '#fff'
                  : '#333',
              }}
            >
            ...
            <button
              ...
              style={{
                background: state.matches({ checkout: { details: { choosingPayment: 'paypal' } } })
                  ? '#1890ff'
                  : '#f5f5f5',
                color: state.matches({ checkout: { details: { choosingPayment: 'paypal' } } })
                  ? '#fff'
                  : '#333',
              }}
            >
