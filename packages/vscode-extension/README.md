...
- **Machine-instance tree** — the **Instances** view is a native tree of running actors (parent → child) with each instance's current state shown; expand an instance to see its **live state-node tree with the active configuration highlighted**. Selecting an instance drives the **Inspector** view (status, active state(s), Send-event controls, persisted-snapshot capture/restore) and the **Context** view — a native, expandable tree of the actor's **real context**
