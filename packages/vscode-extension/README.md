...
- **Machine-instance tree** — the **Instances** view is a native tree of running actors (parent → child) with each instance's current state shown; expand an instance to see its **live state-node tree with the active configuration highlighted**. Selecting an instance drives the **Inspector** view — its status, active state(s), a JSON view of its **real context**, the Send-event controls, and persisted-snapshot capture/restore
