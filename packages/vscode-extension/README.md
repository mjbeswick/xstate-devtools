...
- **Machine-instance tree** — the **Instances** view is a native tree of running actors (parent → child) with each instance's current state shown; expand an instance to see its **live state-node tree with the active configuration highlighted**. Connect/disconnect from the Instances view's title icon. Selecting an instance drives the **Context** view — a native, expandable tree of the actor's **real context**
