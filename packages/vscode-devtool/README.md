...
The extension contributes **Language Model Tools** so VS Code's AI agent can reason about your machines (requires VS Code ≥ 1.95). The tool descriptions steer agents to use them unprompted for any XState question, instead of reading or grepping machine source. You can also `#`-reference them in chat by name: `#xstateMachines` (list machines), `#xstateDescribe` (states + transitions as compact JSON), `#xstateDiagram` (Mermaid), `#xstateTestPaths` (shortest path to each state + unreachable states), `#xstateValidate` (XState diagnostics), `#xstateFindImpl` (locate where an action/guard/service is defined), `#xstateFindRefs` (where a name is *used* as an action/guard/actor/event), `#xstateSetupCoverage` (which actions/guards/actors a machine references and which are missing from `setup()`), `#xstateEvents` (a machine's `send()` API), and `#xstateStateDetail` (one state's entry/exit/invokes + outgoing transitions). Outputs are kept terse to minimise agent token usage. The same analysis is available outside the editor via the standalone [`@xstate-devtools/mcp`](../mcp-server/README.md) server.
